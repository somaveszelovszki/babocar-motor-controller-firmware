...
hw::BridgeDcMotor dcMotor(tim_DC_Motor, timChnl_DC_Motor_Bridge1, timChnl_DC_Motor_Bridge2,
                          cfg::MOTOR_MAX_DUTY);
hw::Encoder encoder(tim_Encoder);
PID_Controller speedController(speedControllerParams, 1.0f, 0.1f, 0.005f);

hw::ServoMotor frontSteeringServo(tim_SteeringServo, timChnl_FrontSteeringServo,
                                  cfg::FRONT_STEERING_SERVO_PWM_CENTER,
                                  cfg::FRONT_STEERING_SERVO_POSITIVE_TRANSFER_RATE,
                                  cfg::FRONT_STEERING_SERVO_NEGATIVE_TRANSFER_RATE,
                                  cfg::FRONT_WHEEL_MAX_DELTA_ANGLE,
                                  cfg::FRONT_SERVO_MAX_ANGULAR_VELOCITY);

hw::ServoMotor rearSteeringServo(tim_SteeringServo, timChnl_RearSteeringServo,
                                 cfg::REAR_STEERING_SERVO_PWM_CENTER,
                                 cfg::REAR_STEERING_SERVO_POSITIVE_TRANSFER_RATE,
                                 cfg::REAR_STEERING_SERVO_NEGATIVE_TRANSFER_RATE,
                                 cfg::REAR_WHEEL_MAX_DELTA_ANGLE,
                                 cfg::REAR_SERVO_MAX_ANGULAR_VELOCITY);
...
template <typename T> bool hasControlTimedOut(const state_t<T>& control) {
    return getTime() - control.timestamp() > millisecond_t(50);
}

bool isSafetySignalOk(const RemoteControllerData& rc) {
    return rc.activeChannel == RemoteControllerData::channel_t::SafetyEnable &&
           isBtw(rc.acceleration, 0.5f, 1.0f);
}

bool isSafetySignalSmoothBrake(const RemoteControllerData& rc) {
    return rc.activeChannel == RemoteControllerData::channel_t::SafetyEnable &&
           isBtw(rc.acceleration, -0.25f, 0.5f);
}

ControlData getControl(const ControlData& swControl,
                       const state_t<RemoteControllerData>& remoteControl) {
    // emergency brake by default
    ControlData control = {{{radian_t(0), radian_t(0), radian_t(0)}},
                           {{m_per_sec_t(0), cfg::EMERGENCY_BRAKE_DURATION}}};

    // remote control must be present, otherwise it means remote controller task or inter-task
    ...
    if (!hasControlTimedOut(remoteControl)) {
        ...
        if (RemoteControllerData::channel_t::DirectControl == rc.activeChannel) {
            control.lat = {micro::lerp(rc.steering, -1.0f, 1.0f, -frontSteeringServo.maxValue(),
                                       frontSteeringServo.maxValue()),
                           micro::lerp(rc.steering, -1.0f, 1.0f, rearSteeringServo.maxValue(),
                                       -rearSteeringServo.maxValue()),
                           radian_t(0)};

            control.lon = {micro::lerp(rc.acceleration, -1.0f, 1.0f, -cfg::DIRECT_CONTROL_MAX_SPEED,
                                       cfg::DIRECT_CONTROL_MAX_SPEED),
                           millisecond_t(0)};
            ...
            if (!useSafetyEnableSignal || isSafetySignalOk(rc)) {
                ...
                // enables lateral software control, but does not change default longitudinal
                // behaviour, which is emergency brake
                control.lat = swControl.lat;
                control.lon = {{m_per_sec_t(0), isSafetySignalSmoothBrake(rc)
                                                    ? cfg::SMOOTH_BRAKE_DURATION
                                                    : cfg::EMERGENCY_BRAKE_DURATION}};
            }
...
void initializeVehicleCan() {
    vehicleCanFrameHandler.registerHandler(
        can::LateralControl::id(), [](const uint8_t* const data) {
            LateralControl lateral;
            reinterpret_cast<const can::LateralControl*>(data)->acquire(
                lateral.frontWheelAngle, lateral.rearWheelAngle, lateral.extraServoAngle);
            swControl.lat = lateral;
        });

    vehicleCanFrameHandler.registerHandler(
        can::LongitudinalControl::id(), [](const uint8_t* const data) {
            LongitudinalControl longitudinal;
            reinterpret_cast<const can::LongitudinalControl*>(data)->acquire(
                longitudinal.speed, useSafetyEnableSignal, longitudinal.rampTime);
            swControl.lon = longitudinal;
        });

    vehicleCanFrameHandler.registerHandler(
        can::SetMotorControlParams::id(), [](const uint8_t* const data) {
            reinterpret_cast<const can::SetMotorControlParams*>(data)->acquire(
                speedControllerParams.P, speedControllerParams.I);
            speedController.tune(speedControllerParams);
            vehicleCanManager.send<can::MotorControlParams>(
                vehicleCanSubscriberId, speedControllerParams.P, speedControllerParams.I);
        });

    const CanFrameIds rxFilter = vehicleCanFrameHandler.identifiers();
    const CanFrameIds txFilter = {can::MotorControlParams::id(), can::LateralState::id(),
...
extern "C" void runControlTask(void) {
    ...
    while (true) {
        ...
        speedController.target = speedRamp
                                     .update(car.speed, validControl.lon.value().speed,
                                             validControl.lon.value().rampTime)
                                     .get();
        ...
        const bool isRemoteControlled =
            remoteControl.value().activeChannel == RemoteControllerData::channel_t::DirectControl;
        vehicleCanManager.periodicSend<can::LongitudinalState>(vehicleCanSubscriberId, car.speed,
                                                               isRemoteControlled, car.distance);
        vehicleCanManager.periodicSend<can::LateralState>(
            vehicleCanSubscriberId, car.frontWheelAngle, car.rearWheelAngle, radian_t(0));
